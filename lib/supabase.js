...
// 온보딩 상태 업데이트
export const updateOnboardingStatus = async (status, dontShowAgain = false) => {
  try {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) {
    ...
    const updateData = {
      ...
      onboarding_dont_show_again: dontShowAgain
    };
...
export const checkOnboardingStatus = async () => {
  try {
    ...
    const { data, error } = await supabase
      .from('users')
      .select('onboarding_completed, onboarding_dismissed, onboarding_viewed_at, onboarding_dont_show_again')
      .eq('id', session.user.id)
      .single();
